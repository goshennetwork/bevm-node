...
var (
	...
	BtcRpcHost = cli.StringFlag{
		Name:  "btc.host",
		Usage: "btc rpc host",
		Value: "127.0.0.1:12345",
	}
	BtcRpcUser = cli.StringFlag{
		Name:  "btc.user",
		Usage: "btc rpc user",
		Value: "",
	}
	BtcRpcPass = cli.StringFlag{
		Name:  "btc.pass",
		Usage: "btc rpc pass",
		Value: "",
	}
...
func SetBtcRpcConfig(ctx *cli.Context, cfg *bevm.BtcRpcConfig) {
	cfg.Host = ctx.GlobalString(BtcRpcHost.Name)
	cfg.User = ctx.GlobalString(BtcRpcUser.Name)
	cfg.Pass = ctx.GlobalString(BtcRpcPass.Name)
}
...
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config, rpcConfig *bevm.BtcRpcConfig) (ethapi.Backend, *bevm.Ethereum) {
	backend, err := bevm.New(stack, cfg, rpcConfig)
	if err != nil {
